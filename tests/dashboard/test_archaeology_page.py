"""Smoke AppTest for the dashboard Archaeology page.
...
def _seed_three_sessions(db: Path) -> None:
    ...
    apply_schema(conn)
    rows = [
        ("S1", 0, datetime(2026, 6, 10, 9, 0), "/proj/a", "claude-opus-4-7", ["Read"]),
        ("S1", 1, datetime(2026, 6, 10, 9, 5), "/proj/a", "claude-opus-4-7", ["Read", "Edit"]),
        ("S2", 0, datetime(2026, 6, 11, 12, 0), "/proj/b", "claude-sonnet-4-6", ["Bash"]),
        ("S3", 0, datetime(2026, 6, 12, 14, 0), "/proj/b", "claude-sonnet-4-6", ["Read", "Bash"]),
    ]
    for session_id, idx, ts, cwd, model, tools in rows:
        conn.execute(
            """
            INSERT INTO turns (
            ...
            """,
            [session_id, idx, ts, cwd, model, len(tools), tools],
        )
    for session_id, cwd, n in [("S1", "/proj/a", 2), ("S2", "/proj/b", 1), ("S3", "/proj/b", 1)]:
        conn.execute(
...
def test_renders_charts_with_three_sessions(
    ...
    assert "Session length distribution" in headers
    assert "Tool co-occurrence" in headers
    assert "Sidechain ratio by session" in headers
    assert "Project activity by day" in headers
